...
@Component({
  ...
  template: `
    <div (mouseenter)="pause()" (mouseleave)="play()" class="carousel slide">
      <ol class="carousel-indicators" *ngIf="slides.length > 1 && !noIndicators">
         <li *ngFor="let slidez of slides" [class.active]="slidez.active === true" (click)="select(slidez)"></li>
      </ol>
      <div class="carousel-inner"><ng-content></ng-content></div>
      <a class="left carousel-control" (click)="prev()" *ngIf="slides.length && !noControls">
        <span class="icon-prev" aria-hidden="true"></span>
        <span *ngIf="isBS4" class="sr-only">Previous</span>
      </a>
      <a class="right carousel-control" (click)="next()" *ngIf="slides.length && !noControls">
...
export class CarouselComponent implements OnDestroy {
  ...
  @Input() public noIndicators:boolean;
  @Input() public noControls:boolean;
